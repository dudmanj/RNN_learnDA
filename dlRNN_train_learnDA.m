...
% parameters
% max_delta_J = 1e-4; % original param
max_delta_J = 0.01;
dt          = 1;
tau         = 30;
dt_div_tau  = dt/tau;
alpha_R     = 0.9;
alpha_X     = 0.33;
% eta_J       = 5e-6+randperm(15,1)/1e5     % this range is most stable for learning
eta_J       = 1e-4;     % this range is most stable for learning
eta_wIn     = 1./tau_trans;
plant_scale = 40;
net_run.eta_J = eta_J;

update      = 4;           % How frequently one should monitor learning
...
for cond = 1:length(target_list)
    ...
    [outputs,hidden_r,hidden_x,e,e_store] = dlRNN_engine(-1,net_out,curr_input,curr_target,act_func_handle,learn_func_handle,transfer_func_handle,0);    
        ...
        for qq=1:error_reps
            ...
            err_vector(qq) = opp_cost * cost * (  1-exp(-deltaRew/250) ) + (cost * sum(abs(diff(outputs(1,500:1600)))) * w_var); % penalizing oscillatory solutions
...
while pass <= 800 % stop when reward collection is very good
    ...
    for cond = 1:length(target_list)
        ...
        for qq=1:error_reps
            ...
            err = opp_cost * cost * (  1-exp(-deltaRew/500) ) + (cost * sum(abs(diff(outputs(1,500:1600)))) * w_var); % penalizing oscillatory solutions
...
% Examine e decay properties
% tmp8(1:size(curr_input,2))  = e_store(50,50,:);
% figure(400); hold off; plot(tmp8); hold on; plot( [round(median(lat_lck)+rewTime) round(median(lat_lck)+rewTime)] , [-1 1], 'r--');

        % Maintain original connectivity sparsity
        e = e';
        e(orig_J == 0) = 0;        
        delta_J = -eta_J .* e .* (R_curr(curr_cond) - R_bar(curr_cond));
...
% figure(300); imagesc(delta_J,[-max_delta_J max_delta_J]); colormap(cm); drawnow;
% figure(300); imagesc(delta_wIn,[-0.2 0.2]); colormap(cm); drawnow;
...
%                 err = opp_cost * cost * (  1-exp(-deltaRew/500) ) + (cost * sum(abs(diff(outputs)) * w_var); % penalizing oscillatory solutions
                err = opp_cost * cost * (  1-exp(-deltaRew/500) ) + (cost * sum(abs(diff(outputs(1,500:1600)))) * w_var); % penalizing oscillatory solutions
            ...
            if monitor
                ...
                plot(conv(sum_lcks,lck_gauss,'same'),'k-');
        ...
        % used to decide whether to continue learning on this condition
...
%-------- ESTIMATE DA response using the Coddington & Dudman 2018 formalism
        % probe network to see whether sensory response is getting enhanced
        % internally within the network and get consensus lick state transition
        
        %*** SWITCH TO INCORPORATE REWARD INTO THIS CALCULATION ****
%                     % Save predicted error
%             R_curr(curr_cond)   = mean(err_vector);
%             R_bar_prev(curr_cond) = R_bar(curr_cond);
%             R_bar(curr_cond) = alpha_R * R_bar_prev(curr_cond) + (1.0 - alpha_R) * R_curr(curr_cond);  

        %*** ALSO NEED TO INCORPORATE STIM LICK+ and LICK- PERTURBATION EFFECTS IN HERE ****
%           Pretty simple should just be a 2x multiplier depending upon detection of anticip licks 
